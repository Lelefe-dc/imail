...
class _ExternalMailboxScreenState extends State<ExternalMailboxScreen> {
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      Mailbox inbox;
      final knownInbox = boxes.where((box) => box.isInbox).toList();
      if (knownInbox.isNotEmpty) {
        inbox = knownInbox.first;
        await client.selectMailbox(inbox);
      } else {
        inbox = await client.selectInbox();
      }
      ...
      if (mounted) {
        setState(() => _error = 'Could not connect to ${widget.account.email}.\n$e');
      }
    } catch (e) {
  ...
  Future<void> _loadSelected() async {
    ...
    final items = await client.fetchMessages(
      mailbox: mailbox,
      count: 50,
  ...
  Future<void> _refresh({bool silent = false}) async {
    if (_client == null || _selected == null) return;
  ...
  String _snippet(MimeMessage item) {
    ...
    if (text.trim().isEmpty) {
      text = (item.decodeTextHtmlPart() ?? '').replaceAll(RegExp(r'<[^>]*>'), ' ');
    }
  ...
  Future<void> _open(MimeMessage item) async {
    ...
    var message = item;
    try {
      message = await client.fetchMessageContents(item, markAsSeen: true);
      item.isSeen = true;
      if (mounted) setState(() {});
    } catch (_) {}
    if (!mounted) return;
              ...
              child: Row(
                children: [
                  ...
                  CircleAvatar(
                    ...
                    child: Text(
                      widget.account.email.substring(0, 1).toUpperCase(),
                                  ...
                                  children: [
                                    CircleAvatar(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(
                                          color: item.isSeen ? imailGreen : Colors.white,
